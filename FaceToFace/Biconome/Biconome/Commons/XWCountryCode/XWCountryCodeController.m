...
//        [self.navigationController.navigationBar setHidden:YES];
        //请输入国名或国际区号进行搜索
        WEAK_SELF
        RSDSearchNavigation * searchNav = [[RSDSearchNavigation alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kNavBar_Height) WithName:LAN(@"搜索") WithSearchResult:^(NSString * _Nonnull str) {
                    NSLog(@"---%@",str);
...
//        [self.navigationController.navigationBar setHidden:NO];
        ...
        if (self.isActive) {
               ...
               BaseViewController *addressInfoVC;
                   for (BaseViewController* vc in self.navigationController.viewControllers) {
                       if ([vc isKindOfClass:[BICAddAddressInfoViewController class]]) {
                           addressInfoVC = (BICAddAddressInfoViewController*)vc;
                       }
                       if ([vc isKindOfClass:[BICAddIdentifyInfoViewController class]]) {
                                addressInfoVC = (BICAddAddressInfoViewController*)vc;
                        }
            ...
            if (addressInfoVC) {
                [self.navigationController popToViewController:addressInfoVC animated:YES];
            }
        }else{
            [self.navigationController popToViewController:self.isWhiteVC animated:YES];
        }
