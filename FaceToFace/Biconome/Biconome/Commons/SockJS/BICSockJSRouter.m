...
//@property(nonatomic,assign)Boolean isDisConnectByBackground;
@end
...
{
    [self reConnectByMySelf];
}

-(void)SockJSGlobeStop
{
    [self.buySellClient disconnect];
    [self.otherClient disconnect];
}
                        ...
                        Type:(NSString*)type
{
    if (sockJsType==SockJS_Type_Market) {
        ...
        RSDLog(@"MarketJs_source--");
        
//        SDUserDefaultsSET(@(responseM.data.cnyAmount.doubleValue), BICUSDTYANG);
//        NSLog(@"BICUSDTYANG======%.2f",responseM.data.cnyAmount.doubleValue);
    } else if (sockJsType==SockJS_Type_BuySellClient)
    {
...
{
    if (sockJS_Type ==SockJS_Type_BuySellClient)
    {
...
//        NSLog(@"盘口订阅地址:%@",[NSString stringWithFormat:@"/topic/subscription/%@",currencyP]);
    ...
    }else if (sockJS_Type==SockJS_Type_Market) {
        return @"/topic/market";
    }
