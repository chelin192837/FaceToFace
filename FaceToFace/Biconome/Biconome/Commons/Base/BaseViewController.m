                   ...
                   ButtonHeight:(CGFloat)buttonHeight
{
    if (!_emptyPlaceHolderView) {
        ...
        mainTitleL.textColor = KThemeText2Color ;
        if (mainTitle) {
        ...
        subTitleL.textColor = KThemeText2Color ;
        if (subtitle) {
        ...
        button.layer.borderColor = ((UIColor *)KThemeText2Color).CGColor;
        [button addTarget:self action:@selector(EmptyButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [button setTitle:buttonTitle forState:UIControlStateNormal];
        [button setTitleColor:KThemeText2Color forState:UIControlStateNormal];
...
- (UIView*)emptyPlaceHolderView2:(UIView *)currentView
                      ImageName:(NSString*)imageName
                    ImageMargin:(CGFloat)imageMargin
                      MainTitle:(NSString*)mainTitle
                MainTitleMargin:(CGFloat)mainTitleMargin
                       ...
                       ViewWidth:(CGFloat)viewwidth
{
    if (!_emptyPlaceHolderView) {
        ...
        _emptyPlaceHolderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, viewwidth, kScreenHeight - kNavBar_Height)];
        ...
        if(imageName){
            imageView.image=[UIImage imageNamed:imageName];
            imageView.frame=CGRectMake((viewwidth-160)/2, imageMargin, 160, 160);
            [_emptyPlaceHolderView addSubview:imageView];
        }
        ...
        mainTitleL.textColor = KThemeText2Color ;
        if (mainTitle) {
            mainTitleL.frame=CGRectMake((viewwidth-mainTitleWidth)/2, CGRectGetMaxY(imageView.frame)+mainTitleMargin, mainTitleWidth, 20);
            [_emptyPlaceHolderView addSubview:mainTitleL];
        }
        ...
        subTitleL.textColor = KThemeText2Color ;
        if (subtitle) {
            subTitleL.frame=CGRectMake((viewwidth-subTitleWidth)/2, CGRectGetMaxY(mainTitleL.frame)+subtitleMargin, subTitleWidth, 20);
            [_emptyPlaceHolderView addSubview:subTitleL];
        }
        ...
        button.layer.masksToBounds = YES ;
        if (buttonTitle) {
            button.frame=CGRectMake((viewwidth-buttonWidth)/2, CGRectGetMaxY(subTitleL.frame)+buttonMargin, buttonWidth, buttonHeight);
            [_emptyPlaceHolderView addSubview:button];
        }
    }
    return _emptyPlaceHolderView;
...
{
    currentView.backgroundColor = KThemeBGColor ;
...
- (void)setupEmptyPlaceHolderView2:(UIView *)currentView
                        ImageName:(NSString*)imageName
                      ImageMargin:(CGFloat)imageMargin
                        MainTitle:(NSString*)mainTitle
                  MainTitleMargin:(CGFloat)mainTitleMargin
...
{
    currentView.backgroundColor = KThemeBGColor ;
    [currentView addSubview:[self emptyPlaceHolderView2:currentView ImageName:imageName ImageMargin:imageMargin MainTitle:mainTitle MainTitleMargin:mainTitleMargin MainTitleWidth:mainTitleWidth Subtitle:subtitle SubtitleMargin:subtitleMargin SubTitleWidth:subTitleWidth Button:buttonTitle ButtonMargin:buttonMargin ButtonWidth:buttonWidth ButtonHeight:buttonHeight ViewWidth:width]];
}
...
- (void)setupNoDataOfSearchBiconome:(UIView *)currentView With:(CGFloat)topMargin
{
    if (![self.noDataOfSearch isMemberOfClass:[currentView class]]) {
        currentView.backgroundColor = KThemeBGColor ;
        currentView.superview.backgroundColor = KThemeBGColor ;
        [self noDataOfSearchBiconome:currentView With:topMargin];
    }
}

- (void)setupNoDataOfSearchBiconome2:(UIView *)currentView With:(CGFloat)topMargin
{
    if (![self.noDataOfSearch isMemberOfClass:[currentView class]]) {
        currentView.backgroundColor = [UIColor whiteColor] ;
        currentView.superview.backgroundColor = [UIColor whiteColor];
        [self noDataOfSearchBiconome2:currentView With:topMargin];
    }
}
...
{
    if (!_noDataOfSearch) {
        _noDataOfSearch = [[UIView alloc] initWithFrame:CGRectMake(0, 0, currentView.frame.size.width, currentView.height)];
        _noDataOfSearch.backgroundColor = KThemeBGColor;
        [currentView addSubview:_noDataOfSearch];
 ...
 - (UIView *)noDataOfSearchBiconome2:(UIView *)currentView With:(CGFloat)topMargin
 {
     if (!_noDataOfSearch) {
         _noDataOfSearch = [[UIView alloc] initWithFrame:CGRectMake(0, 0, currentView.frame.size.width, currentView.height)];
         _noDataOfSearch.backgroundColor = [UIColor whiteColor];
