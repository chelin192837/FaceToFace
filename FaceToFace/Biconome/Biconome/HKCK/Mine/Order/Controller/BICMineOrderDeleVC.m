...
- (void)viewDidLoad {
    ...
    [self initNavigationRightButtonWithTitle:LAN(@"取消全部") titileColor:kNVABICSYSTEMTitleColor];
...
{
    BICCancelView *view=[[BICCancelView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, KScreenHeight) title:LAN(@"提示") content:LAN(@"确定取消全部订单？") left:LAN(@"再想想") right:LAN(@"确认")];
    WEAK_SELF
...
-(void)setupData:(BICListUserRequest*)request
{
    ...
    [[BICExchangeService sharedInstance] analyticalPCListUserOrderData:request serverSuccessResultHandler:^(id response) {
        ...
        if (weakSelf.dataArray.count == 0) {
            ...
            [weakSelf initNavigationRightButtonWithTitle:LAN(@"取消全部") titileColor:kNVABICSYSTEMTitleColor];
            weakSelf.navigationRightBtn.hidden = NO;
        }
        [weakSelf.tableView reloadData];
