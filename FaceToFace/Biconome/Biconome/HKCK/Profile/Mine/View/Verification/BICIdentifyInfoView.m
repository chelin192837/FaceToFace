...
-(void)layoutSubviews{
    ...
    self.longtimeButton.frame=CGRectMake(KScreenWidth-32-50, CGRectGetMaxY(self.postVLabel.frame)+24, 50, 20);
}
...
-(void)setResponse:(BICAuthInfoResponse *)response{
    ...
    if(response.data.cardLastTime){
        self.longtimeButton.hidden=YES;
        self.detailVLabel.hidden=NO;
    }else{
        self.longtimeButton.hidden=NO;
        self.detailVLabel.hidden=YES;
    }
...
-(UIButton *)longtimeButton{
    if(!_longtimeButton){
        ...
        [_longtimeButton setImage:[UIImage imageNamed:@"CheckboxSelected"] forState:UIControlStateNormal];
...
//        [_longtimeButton setTitleColor:rgba(100, 102, 108, 1) forState:UIControlStateSelected];
        ...
        _longtimeButton.userInteractionEnabled=NO;
