...
-(void)setSelectPageType:(SelectPage_Type)selectPageType
{
    ...
    if (selectPageType==SelectPage_Type_Language) {
        ...
        }else if([[BICDeviceManager getLanguageCurrencyStr] isEqualToString:@"ru"]){
            self.selIndex = [NSIndexPath indexPathForRow:2 inSection:0];
        }else if([[BICDeviceManager getLanguageCurrencyStr] isEqualToString:@"ko"]){
            self.selIndex = [NSIndexPath indexPathForRow:3 inSection:0];
        }else{ //英文
    ...
    if (selectPageType==SelectPage_Type_theme) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_selectPageType==SelectPage_Type_Language) {
        if (indexPath.row==0) { //英文
            [[NSUserDefaults standardUserDefaults] setObject:@"en" forKey:@"language_type"];
        }
        if (indexPath.row==1) { //中文
            [[NSUserDefaults standardUserDefaults] setObject:@"zh-Hans" forKey:@"language_type"];
        }
        if (indexPath.row==2) { //饿文
            [[NSUserDefaults standardUserDefaults] setObject:@"ru" forKey:@"language_type"];
        }
        if (indexPath.row==3) { //han文
           [[NSUserDefaults standardUserDefaults] setObject:@"ko" forKey:@"language_type"];
        }
    ...
    if (_selectPageType==SelectPage_Type_theme) {
        if (indexPath.row==0) { //white
            [[NSUserDefaults standardUserDefaults] setObject:@"white" forKey:BICThemeConfigType];
        }
        if (indexPath.row==1) { //black
            [[NSUserDefaults standardUserDefaults] setObject:@"black" forKey:BICThemeConfigType];
        }
        if (indexPath.row==2) { //blue
            [[NSUserDefaults standardUserDefaults] setObject:@"blue" forKey:BICThemeConfigType];
        }
