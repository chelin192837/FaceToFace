...
-(void)setupUI
{
    ...
    self.middenBgView.userInteractionEnabled=YES;
    [self.middenBgView.rightButton addTarget:self action:@selector(changeSort) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (_response) {
        ...
        if((45+32)*self.titleArray.count+100 > SCREEN_WIDTH){
                self.titleCollectionView.width = (45+32)*self.titleArray.count+500;
//                self.titleCollectionView.contentSize=CGSizeMake((45+32)*self.titleArray.count+100+50, 0);
        }
...
-(void)changeSort{
    self.middenBgView.rightButton.selected=!self.middenBgView.rightButton.selected;
    [self.contentCollectionView reloadData];
}
...
//            cell.pointView.hidden =YES;
        ...
        vc.isSort=self.middenBgView.rightButton.selected;
...
//        CGFloat collectionViewWidth = CGRectGetWidth(self.titleCollectionView.frame);
//            UICollectionViewCell *cell = [self.titleCollectionView cellForItemAtIndexPath:indexPath];
//        if(CGRectGetMaxX(cell.frame)>SCREEN_WIDTH){
//            CGPoint offset = CGPointMake(CGRectGetMaxX(cell.frame)-SCREEN_WIDTH+16, 0);
//            [self.titleCollectionView setContentOffset:offset animated:YES];
//        }
