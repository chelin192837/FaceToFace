...
-(void)updateUI
{
    ...
    self.headV.searchLab.textColor=KThemeText2Color;
}
-(void)setupUI
{
    BICMSearchHeaderView* headV = [[NSBundle mainBundle] loadNibNamed:@"BICMSearchHeaderView" owner:nil options:nil][0];
    headV.backgroundColor=KThemeBGColor;
    headV.contentView.backgroundColor=KThemeText6Color;
    self.headV=headV;
    headV.searchLab.text = LAN(@"搜索");
    headV.searchLab.backgroundColor=KThemeText6Color;
    ...
    if([[BICDeviceManager getCurrentTheme]isEqualToString:@"white"]){
        headV.searchImage.image=[UIImage imageNamed:@"nav_search_dark"];
    }else{
        headV.searchImage.image=[UIImage imageNamed:@"nav_search_light"];
    }
