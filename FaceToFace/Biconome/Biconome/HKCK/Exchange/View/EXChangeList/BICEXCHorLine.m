...
-(instancetype)initWithFrame2:(CGRect)frame With:(HorLineType)type
{
    if (self=[super initWithFrame:frame]) {
        [self setupUI2:frame With:type];
    }
    return self;
}
...
-(void)setupUI2:(CGRect)frame With:(HorLineType)type
{
    self.type=type;
    for (int i=0; i<5; i++) {
        BICEXCRightLine *line = [[BICEXCRightLine alloc] initWithNibWith2:type];
        line.frame= CGRectMake(0,i * 28, frame.size.width, 28);
        [self addSubview:line];
    }
}
...
-(void)setArr2:(NSArray *)arr
{
    ...
    if (self.type == HorLineType_Green) {
        for (int i=0; i<count; i++) {
            BICEXCRightLine *line = [[BICEXCRightLine alloc] initWithNibWith2:self.type];
            line.frame= CGRectMake(0,i * 28, self.frame.size.width, 28);
            if(self.isRever){
                [line setModelRever:arr[i]];
                [line setupUIRever:self.type];
