...
//-(void)sendPing
//{
//    NSString * price = [NSString stringWithFormat:@"%f",[BICDeviceManager getRandomNumber:9280 to:9300]];
//    NSLog(@"sendPing--%@",price);
...
-(void)SockJSKLine:(Y_KLineModel *)lineModel Type:(NSString*)type
{
    if (lineModel) {
        if (self.groupModel != nil) {
            ...
            WEAK_SELF
              Y_KLineModel *lineModelLast = [self.groupModel.models lastObject];
                   
                                lineModelLast = lineModel;
 
                                   NSDate *date = [NSDate dateWithTimeIntervalSince1970:lineModelLast.Date.doubleValue/1000 + self.type.integerValue * 60];
                                   BOOL  isresh  = [self compareDate:[NSDate date] withDate:date];
                                   
                                   if (!isresh) { //如果不刷新
                                       lineModelLast.Date = [NSString stringWithFormat:@"%@",@(lineModelLast.Date.doubleValue +1)];
                                       [self reloadData];
                                   }else
                                   {
                                   }
                                   NSString *price = NSStringFormat(@"%@",lineModelLast.Close);
                                  NSLog(@"price--price--%@",price);
...
//    lineBGView.layer.masksToBounds = YES;
    ...
    rightTL.text = [NSString stringWithFormat:@"%@\n%@\n%@",LAN(@"24H量"),LAN(@"24H高"),LAN(@"24H低")];
...
//            [innnerArr addObject:model.timestamp];
            if (model.timestamp.length>3) {
                [innnerArr addObject:[model.timestamp substringToIndex:model.timestamp.length-3]];
            }
...
//        Y_KLineGroupModel *groupModel = [Y_KLineGroupModel objectWithArray:mulArr];
        Y_KLineGroupModel *groupModel = [Y_KLineGroupModel objectWithArray:[self formatArr:mulArr Type:self.type]];
...
//格式化处理,在指定时间类型里，时间出现断裂，那么设置成挂单状态
-(NSMutableArray*)formatArr:(NSMutableArray*)array Type:(NSString*)type
{
    long typeInt = type.longLongValue * 60 ;
    
    NSMutableArray * arrFir = [array firstObject];
    
//    NSMutableArray * arrLast = [array lastObject];
    
    long modelFirInt = [arrFir[0] longLongValue];
    
//    long modelLastInt = [arrLast[0] longLongValue];
    
    NSDate *date = [NSDate date];
    
    NSTimeInterval interval = [date timeIntervalSince1970]; //当前时间的时间戳
    
    long modelLastInt = (long)interval;
    
    long num ; //应该有的蜡烛个数
    if ((modelLastInt - modelFirInt)%typeInt==0) {
        num = (modelLastInt - modelFirInt)/typeInt; //应该有的蜡烛个数
    }else{
        num = (modelLastInt - modelFirInt)/typeInt + 1 ; //应该有的蜡烛个数
    }
    
    NSMutableArray * arrAll = [NSMutableArray array];
    for (int i=0; i<num; i++) {
        NSMutableArray * arr = [NSMutableArray arrayWithArray:arrFir.copy];
        [arr replaceObjectAtIndex:0 withObject:[NSString stringWithFormat:@"%ld",(modelFirInt + i * typeInt)]];
        
        [arrAll addObject:arr];
    }
    
    for (int i=0; i<arrAll.count; i++) { //在后台传的数据中遍历 81
        NSMutableArray * arrUpper = arrAll[i];
        BOOL index = YES;
        for (int j=0; j<array.count; j++) {
            NSMutableArray * arrInner = array[j];
            //如果能找到
            if ([[arrUpper firstObject] longLongValue] == [[arrInner firstObject] longLongValue]) {
                [arrAll replaceObjectAtIndex:i withObject:arrInner];
                index = NO ;
            }
        }
        //如果找不到，将蜡烛变成一条线
        if (index) {
            ...
            NSMutableArray* arrUpdate = [NSMutableArray arrayWithArray:arrAll[i-1]];
            [arrUpdate replaceObjectAtIndex:1 withObject:arrUpdate[4]];
            [arrUpdate replaceObjectAtIndex:2 withObject:arrUpdate[4]];
            [arrUpdate replaceObjectAtIndex:3 withObject:arrUpdate[4]];
            [arrUpdate replaceObjectAtIndex:4 withObject:arrUpdate[4]];
            [arrUpdate replaceObjectAtIndex:5 withObject:@"0"];

            [arrAll replaceObjectAtIndex:i withObject:arrUpdate];
            
        }
        
        
        NSMutableArray * arrTimestamp = arrAll[i];
        if ([arrTimestamp[5] longLongValue]!=0) {
            NSString * timestamp = [NSString stringWithFormat:@"%@%@",arrTimestamp[0],@"000"];
                  [arrTimestamp replaceObjectAtIndex:0 withObject:timestamp];
        }
