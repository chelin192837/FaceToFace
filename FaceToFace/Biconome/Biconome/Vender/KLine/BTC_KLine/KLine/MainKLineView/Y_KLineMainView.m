...
- (void)drawMaxAndMinValueAndCurrentPrice{
    ...
    {
        model.Close = [NSNumber numberWithFloat:_price.floatValue];
    }
...
//        y = ABS(maxY - (model.Close.doubleValue - low.doubleValue)/unitValue);
    ...
    //横屏实时价格的x坐标 减50（指标宽度）--77-- SafeAreaBottomHeight
    [self.currentPriceView setFrame:CGRectMake(self.isFullScreen?(priceRectx+5):(priceRectx+5), y - 7.5, Y_StockChartKLinePriceViewWidth, 15)];
//        [self.currentPriceView setFrame:CGRectMake(self.isFullScreen?(priceRectx+5):(256.f), y - 7.5, Y_StockChartKLinePriceViewWidth, 15)];
    ...
    self.currentPriceView.x = 256.f;
    
    NSLog(@"self.currentPriceView.x--%f",self.currentPriceView.x);
